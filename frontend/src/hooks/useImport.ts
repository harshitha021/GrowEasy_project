...
export interface ImportState {
  ...
  /** History entry id backing the current results view (for retry merging). */
...
export function useImport() {
   ...
   * With a `merge` target, results are merged back into the original import
   * instead of becoming a separate one.
   */
  const launchJob = useCallback(async (rows: RawRow[], fileName: string, merge?: MergeTarget) => {
    setState((s) => ({ ...s, step: 'importing', error: null, progress: null, fileName }));
    try {
      const jobId = await startImport(rows);
      if (merge) {
        addRetryEntry(jobId, fileName, rows.length, merge.originId, merge.retryMap);
      } else {
      ...
      unsubscribeRef.current = subscribeToJob(jobId, {
        ...
        onDone: (summary) => {
          recordJobCompletion(jobId, summary);
          const finalSummary = merge
            ? mergeSummaries(merge.baseSummary, remapSummary(summary, merge.retryMap))
            : summary;
          const finalFileName = merge ? merge.originFileName : fileName;
          const historyId = merge ? merge.originId : jobId;
          if (watchedJobRef.current !== jobId) return;
          setState((s) =>
            s.step === 'importing'
              ? {
   ...
   * Results merge back into the original import — not a separate one.
   */
  const retrySkipped = useCallback(async () => {
    const summary = state.summary;
    const skipped = summary?.skipped;
    if (!summary || !skipped || skipped.length === 0) return;
    const merge: MergeTarget | undefined = state.historyId
      ? {
          originId: state.historyId,
          originFileName: state.fileName,
    ...
    await launchJob(
      skipped.map((s) => s.raw),
      `Retry - ${state.fileName || 'import.csv'}`,
      merge
    );
